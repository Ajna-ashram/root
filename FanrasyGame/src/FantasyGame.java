...
//        com2 = new Orks();//testing row
    ...
    public void run(){
        ...
        int Round = 1;
        while (!isGameOver){
            FantasyGame.Print_Log("------Раунд: "+ Round++ +"------");
            //FantasyGame.Print_Log("Ход привилегированных персоонажей");
            //stepPrem(com1,com2);
            //stepPrem(com2,com1);
            FantasyGame.Print_Log("------Битва------");
            step(com1,com2);
            if (com1.avalibleCreatureCount()==0){
                isGameOver=true;
                FantasyGame.Print_Log("Победа команды - " + com2.getgName());
            }
            else if(com2.avalibleCreatureCount()==0){
                isGameOver=true;
                FantasyGame.Print_Log("Победа команды - " + com1.getgName());
            }
            RenewDoStep(com1);
            RenewDoStep(com2);

        }
        FantasyGame.Print_Log("------Game Over------");
            ...
            for (Creature c : c1.unit) {
                if (c.isPremium()&&c.isAvalible()) {
                    if (c instanceof Elf.MagElf||c instanceof People.MagHuman)
                    {
                        ...
                        if(random.nextInt(2)==0) c.turn(randomCreature(c1));
                        else c.turn(randomCreature(c2));
                    }
                        ...
                        if(random.nextInt(2)==0) c.turn(randomCreature(c2));
                        else c.turn(randomCreature(c2));
                    }
    ...
    public void step (Generation c1, Generation c2){
        ...
        //FantasyGame.Print_Log("------Боевой стек сформирован------"); //temporary
        while (batleStek.peek()!=null){
            ...
            if(c1.unit.contains(doStepCreature)&&doStepCreature.isAvalible()){//creature from com1 and creature availible
                if(doStepCreature.race.equals(Creature.Race.ELF)||doStepCreature.race.equals(Creature.Race.PEOPLE)){
                    if (random.nextInt(2)==0) doStepCreature.turn(randomCreature(c1));//do spel on friendly creature
                    else doStepCreature.turn(randomCreature(c2)); // atack enemy
                }
                ...
                if(doStepCreature.race.equals(Creature.Race.ELF)||doStepCreature.race.equals(Creature.Race.PEOPLE)){
                    if (random.nextInt(2)==0) doStepCreature.turn(randomCreature(c2));//do spel on friendly creature
                    else doStepCreature.turn(randomCreature(c1));// atack enemy
                }
                else if(doStepCreature.race.equals(Creature.Race.ORKS)){
                    if (random.nextInt(2)==0) doStepCreature.turn(randomCreature(c2));//do spel on friendly creature
                    else doStepCreature.turn(randomCreature(c1));// atack enemy
    ...
    public Creature randomCreature(Generation com){
        ...
        int rIndx = random.nextInt(avalible.unit.size());
    ...
    public static boolean Print_Log(String s){
